...
<body>
	...
	<div class="content">
		...
		<header class="activity">
			...
			<p class="subject"><?php  echo $item['title'];?></p>
		...
		<section id="top" class="activity">
			...
			<?php  } else if($item['begintime'] > TIMESTAMP) { ?>
                <button type="button" class="no-join">
                    <span style='color:#888;'>再等等呢...</span>
                </button>
             <?php  } else if($item['endtime'] < TIMESTAMP) { ?>
             <button type="button" class="no-join">
                    <span style='color:#888;'>已结束</span>
                </button>
                <?php  } else if($item['begintime']< TIMESTAMP&&$item['endtime']>TIMESTAMP) { ?>
		...
		<section id="applicants" class="activity">
			...
			<?php  if(is_array($pfans)) { foreach($pfans as $row) { ?>
				<li>
					<!-- 头像 -->
		...
		<section id="comments" class="activity">
			...
			<div class="article-wrap">
				<!-- 一条评论 START -->
				<?php  if(is_array($pcomments)) { foreach($pcomments as $row) { ?>
				<article>
					<!-- 头像，居左 -->
					<img class="headimg" src="<?php  echo toimage($row['head_img'])?>" />
					<!-- 文字，居右 -->
