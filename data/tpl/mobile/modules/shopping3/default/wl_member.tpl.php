...
<div id="cover_layer"></div>
	<?php  include $this->template(wl_menu, TEMPLATE_INCLUDEPATH);?>
 	<ul id="member_nav">
		...
		<li <?php  if($_GPC['status']==2&&$_GPC['ispay']==1) { ?>class="cur"<?php  } ?>><a href="<?php  echo $this->createMobileUrl('wlmember',array('d'=>'list','status'=>2));?>">已确认</a></li>
	...
	<div id="order_list">
		...
		<div class="item">
			...
			<div class="pro">
				<div class="img">
				<a href="<?php  echo $this->createMobileUrl('wlmember',array('d'=>'detail','status'=>$row['status'],'orderid'=>$row['id']));?>"><img src="<?php  echo $row2['thumb'];?>"></a></div>
				<dl class="info">
					<dd class="name"><a href="<?php  echo $this->createMobileUrl('wlmember',array('d'=>'detail','status'=>$row['status'],'orderid'=>$row['id']));?>"><?php  echo $row2['title'];?></a></dd>
	...
	<div id="order_detail">
			...
			<ul>
				...
				<li>就餐类型: <?php  if($order['order_type']==2) { ?>店内就餐【<?php  if($order['seat_type']==1) { ?>大厅<?php  } else { ?>包房<?php  } ?>桌号:<?php  echo $order['desk'];?>】<?php  } else { ?>外卖外送<?php  } ?></li>
				<?php  if($order['order_type']!=2) { ?><li>就餐类型: <?php  if($order['sendstatus']==1) { ?><strong class="fc_red">已经配送</strong><?php  } else { ?>未配送<?php  } ?> </li><?php  } ?>
 				<li>订单备注: <?php  echo $order['remark'];?></li>
			</ul>
		</div>
		<div class="item">
			<ul>
				...
				<?php  if($order['order_type']==1) { ?><li>地址：<?php  echo $order['guest_address'];?></li><?php  } ?>
			</ul>
		</div>
		...
		<div class="item">
			...
			<div class="pro">
				<div class="img">
				<a href="<?php  echo $this->createMobileUrl('wldetail', array('goodsid' => $row2['id']));?>"><img src="<?php  echo $row2['thumb'];?>"></a></div>
				<dl class="info">
					<dd class="name"><a href="<?php  echo $this->createMobileUrl('wldetail', array('goodsid' => $row2['id']));?>"><?php  echo $row2['title'];?></a></dd>
					<dd>价格:￥<?php  echo $row2['marketprice'];?></dd>
					<dd>数量:<?php  echo $row2['total'];?></dd>
 				</dl>
				<div class="clear"></div>
 ...
 <div id="footer_menu" class="footer footer_menu">
            <ul class="clear">
                <li><a href="<?php  echo  $this->createMobileUrl('wlhome')?>"    <?php  if($_GPC['do']=='wlhome') { ?>class="onactive"<?php  } ?>><span class="icons icons_1"></span><label>店铺详情</label></a></li>
				 <?php  if(($_GPC['do']!='wlmember' && $_GPC['do']!='wlorder'&& $_GPC['do']!='wlmylike' && $_GPC['do']!='wlhome' )) { ?>
				...
				<?php  } ?>				
                <li><a href="<?php  echo  $this->createMobileUrl('wlcart')?>" id="my_menu"><span class="icons icons_5"><label id="num" class="num"><?php  echo $totalnum;?></label></span></a></li>
            </ul>
        </div>
        <script>
