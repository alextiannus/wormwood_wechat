...
class BooktableModuleSite extends WeModuleSite {
	...
	public function doWebMmphoto() {
		...
		$id = intval($_GPC['id']);//获得id
			if(!empty($id)){//获取更新座位数据
				//查找是否存在
				$item = pdo_fetch("SELECT * FROM ".tablename('research_tables')." WHERE id = :id" , array(':id' => $id));
				if (empty($item)) {
			...
			if(checksubmit('submit')){//检测是否post
				...
				if(empty($id)){
					pdo_insert('research_tables', $data);//添加数据
				message('图片添加成功！', $this->createWebUrl('mmphoto', array('op' => 'display')), 'success');
				}else{
					pdo_update('research_tables', $data, array('id' => $id));
			...
			$id = intval($_GPC['id']);
			$row = pdo_fetch("SELECT id FROM ".tablename('research_tables')." WHERE id = :id", array(':id' => $id));
			if (empty($row)) {
				message('亲,你的数据'.$id.'不存在,不要乱动哦！');
			}
			pdo_delete('research_tables', array('id' => $id));
			message('删除成功！', referer(), 'success');	
		}else if('display'==$op){
			$pindex = max(1, intval($_GPC['page']));
			$psize =20;//每页显示
			$list = pdo_fetchall("SELECT * FROM ".tablename('research_tables')."  ORDER BY id DESC LIMIT ".($pindex - 1) * $psize.','.$psize);//分页
