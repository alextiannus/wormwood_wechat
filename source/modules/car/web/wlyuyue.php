...
if($op=='post'){
	...
	if ($_GPC['submit']!=''){
		...
		if($id==0){
			$temp = pdo_insert('we7car_yuyue_set', $insert);
		}else{
			$temp = pdo_update('we7car_yuyue_set', $insert,array('id'=>$id));
		}
		if($temp==false){
 	...
 	if($id>0){
		$item = pdo_fetch("SELECT * FROM ".tablename('we7car_yuyue_set')." WHERE  weid=:weid  AND id={$id}" , array(':weid'=>$_W['weid']));		
	...
	$brand = pdo_fetchall("SELECT * FROM ".tablename('we7car_brand')." WHERE weid = ".$weid."  order by listorder desc");		
	if(!empty($theone['bid'])){
		$series=pdo_fetchall("SELECT id,title FROM ".tablename('we7car_series')." WHERE weid = ".$weid." and bid=".$theone['bid']."  order by listorder desc");					
	}	
	include $this->template('web/yuyue_post');
}elseif($op=='list'){
	$list = pdo_fetchall("SELECT * FROM ".tablename('we7car_yuyue_set')." WHERE weid = ".$weid."  order by id desc");
	...
	if(!empty($_GPC['status'])){
		...
		$id=$_GPC['id'];
		$status=$_GPC['status'];
		$temp=pdo_update('we7car_yuyue_list',array('status'=>$status), array('id' => $id,'weid'=>$weid));
		if($temp==false){
	...
	$list = pdo_fetchall("select * from ".tablename('we7car_yuyue_list')." where weid=".$weid ."  order by id desc LIMIT ".($pindex - 1) * $psize.','.$psize); 
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('we7car_yuyue_list') . "  WHERE weid = '{$weid}' $condition");
