...
class LxybulletinboardModule extends WeModule {
	...
	public $table_reply='lxy_bulletin_board_card_reply';
	
	public function fieldsFormDisplay($rid = 0) {
		//要嵌入规则编辑页的自定义内容，这里 $rid 为对应的规则编号，新增时为 0
		global $_W;
		$weid=$_W['weid'];
		$cards = pdo_fetchall("SELECT id,title FROM ".tablename($this->cardtable)." WHERE weid = :weid ORDER BY `id` DESC", array(':weid' => $weid));
		if (!empty($rid)) {
