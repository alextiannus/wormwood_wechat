...
class LxybuscardperModuleSite extends WeModuleSite {
	public $cardtable='lxy_bussiness_per_card';
	public $coptable='lxy_bussiness_per_card_cop';
	public $classtable='lxy_bussiness_per_card_class';
	public $replytable='lxy_bussiness_per_card_reply';
	public $tplname=array('default','card_yellow','card_bull','card_bull_s','card_deful','card_fas','card_mount');
	public $producttable='lxy_bussiness_per_card_product';
	...
	public function doWebProductadd() {
		...
		if (checksubmit('submit')) {
			...
			message('产品更新成功！', $this->createWebUrl('productlist', array('cid'=>$cid,'busid'=>$busid)), 'success');
		}
	...
	public function doMobileViewcard() {
		...
		$replyTitle=pdo_fetchcolumn("select title from ".tablename($this->replytable)." where cid={$id}");
		...
		$item['copintro']=$copinfo['copintro'];
