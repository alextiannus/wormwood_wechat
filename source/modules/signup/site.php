...
class SignupModuleSite extends WeModuleSite {

	public function getHomeTiles() {
		global $_W;
		$urls = array();
		$list = pdo_fetchall("SELECT title, reid FROM ".tablename('signup')." WHERE weid = '{$_W['weid']}'");
		if (!empty($list)) {
			foreach ($list as $row) {
				$urls[] = array('title'=>$row['title'], 'url'=> $this->createMobileUrl('signup', array('id' => $row['reid'])));
			}
		}
		return $urls;
	}
	...
	public function doWebQuery() {
		global $_W, $_GPC;
		$kwd = $_GPC['keyword'];
		$sql = 'SELECT * FROM ' . tablename('signup') . ' WHERE `weid`=:weid AND `title` LIKE :title ORDER BY reid DESC LIMIT 0,8';
		$params = array();
		$params[':weid'] = $_W['weid'];
		$params[':title'] = "%{$kwd}%";
	...
	public function doWebDetail() {
		...
		if (!empty($_GPC['sta'])) {
			...
			$sql = 'UPDATE ' . tablename('signup_rows') . ' SET `status`=:status WHERE `rerid`=:rerid';
			$params = array();
			$params[':rerid'] = $rerid;
		...
		$sql = 'SELECT * FROM ' . tablename('signup_rows') . " WHERE `rerid`=:rerid";
		$params = array();
		$params[':rerid'] = $rerid;
		...
		$sql = 'SELECT * FROM ' . tablename('signup_fields') . ' WHERE `reid`=:reid ORDER BY `refid`';
		$params = array();
		$params[':reid'] = $row['reid'];
		...
		$fids = implode(',', $fids);
		$row['fields'] = array();
		$sql = 'SELECT * FROM ' . tablename('signup_data') . " WHERE `reid`=:reid AND `rerid`='{$row['rerid']}' AND `refid` IN ({$fids})";
	...
	public function doWebExcel() {
		...
		$sql = 'SELECT * FROM ' . tablename('signup_rows') . " WHERE `reid`=:reid AND `createtime` > {$starttime} AND `createtime` < {$endtime} ORDER BY `createtime` DESC " ;
		$params = array();
		$params[':reid'] = $reid;
		...
		if($select) {
			...
			foreach($list as $key=>&$r) {
				...
				$r['fields'] = array();
				$sql = 'SELECT * FROM ' . tablename('signup_data') . " WHERE `reid`=:reid AND `rerid`='{$r['rerid']}' AND `refid` IN ({$fids})";
				$fdatas = pdo_fetchall($sql, $params);
				$sql1 = 'SELECT * FROM ' . tablename('signup_fields') . " WHERE `reid`=:reid  AND `refid` IN ({$fids})";
  ...
  exit;//include $this->template('a');
	...
	public function doWebManage() {
		...
		$psize = 50;
		$sql = 'SELECT * FROM ' . tablename('signup_rows') . " WHERE `reid`=:reid AND `createtime` > {$starttime} AND `createtime` < {$endtime} ORDER BY `createtime` DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
		$params = array();
		$params[':reid'] = $reid;

		$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('signup_rows') . " WHERE `reid`=:reid AND `createtime` > {$starttime} AND `createtime` < {$endtime}", $params);
		...
		if($select) {
			...
			if(!empty($_GPC['title'])){
				$sql = 'SELECT * FROM ' . tablename('signup_data') . " WHERE `reid`=:reid AND `data`='{$title}' AND `refid` = '{$sou}' ";
	...
	public function doWebDisplay() {
		...
		if($_W['ispost']) {
			...
			$sql = 'UPDATE ' . tablename('signup') . ' SET `status`=:status WHERE `reid`=:reid';
			$params = array();
		...
		$sql = 'SELECT * FROM ' . tablename('signup') . ' WHERE `weid`=:weid';
		$ds = pdo_fetchall($sql, array(':weid' => $_W['weid']));
		foreach($ds as &$item) {
			$item['isstart'] = $item['starttime'] > 0;
			$item['switch'] = $item['status'];
			$item['link'] =  $this->createMobileUrl('signup', array('id' => $item['reid']));
		}
	...
	public function doWebDelete() {
		...
		if($reid > 0) {
			$params = array();
			$params[':reid'] = $reid;
			$sql = 'DELETE FROM ' . tablename('signup') . ' WHERE `reid`=:reid';
			pdo_query($sql, $params);
			$sql = 'DELETE FROM ' . tablename('signup_rows') . ' WHERE `reid`=:reid';
			pdo_query($sql, $params);
			$sql = 'DELETE FROM ' . tablename('signup_fields') . ' WHERE `reid`=:reid';
			pdo_query($sql, $params);
			$sql = 'DELETE FROM ' . tablename('signup_data') . ' WHERE `reid`=:reid';
			pdo_query($sql, $params);
	...
	public function doWebSignupDelete() {
		global $_W, $_GPC;
		$id = intval($_GPC['id']);
		if (!empty($id)) {
			pdo_delete('signup_rows', array('rerid' => $id));
		}
		message('操作成功.', referer());
	}
	...
	public function doWebPost() {
		...
		$hasData = false;
		if($reid) {
			$sql = 'SELECT COUNT(*) FROM ' . tablename('signup_rows') . ' WHERE `reid`=' . $reid;
		...
		if(checksubmit()) {
			...
			if(empty($reid)) {
				...
				if(pdo_update('signup', $recrod, array('reid' => $reid)) === false) {
					message('保存报名失败, 请稍后重试.');
				}
			}
			...
			if(!$hasData) {
				...
				foreach($_GPC['title'] as $k => $v) {
					...
					pdo_insert('signup_fields', $field);
				}
			}
			message('保存报名成功.', 'refresh');
		}
	...
	public function doMobileSignup() {
		...
		if(checksubmit()) {
			$pretotal = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('signup_rows')." WHERE reid = :reid AND openid = :openid", array(':reid' => $reid, ':openid' => $_W['fans']['from_user']));
			...
			if(pdo_insert('signup_rows', $row) != 1) {
				message('保存失败.');
			}
			$rerid = pdo_insertid();
			if(empty($rerid)) {
			...
			foreach($datas as &$r) {
				$r['rerid'] = $rerid;
				pdo_insert('signup_data', $r);
			}
			if(empty($activity['starttime'])) {
				$record = array();
				$record['starttime'] = TIMESTAMP;
				pdo_update('signup', $record, array('reid' => $reid));
			}
			//发送报名
	...
	public function doMobileMySignup() {
		global $_W, $_GPC;
		$operation = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
		if ($operation == 'display') {
			$rows = pdo_fetchall("SELECT * FROM ".tablename('signup_rows')." WHERE openid = :openid", array(':openid' => $_W['fans']['from_user']));
			if (!empty($rows)) {
				foreach ($rows as $row) {
					$reids[$row['reid']] = $row['reid'];
				}
				$signup = pdo_fetchall("SELECT * FROM ".tablename('signup')." WHERE reid IN (".implode(',', $reids).")", array(), 'reid');
			}
		} elseif ($operation == 'detail') {
			$id = intval($_GPC['id']);
			$row = pdo_fetch("SELECT * FROM ".tablename('signup_rows')." WHERE openid = :openid AND rerid = :rerid", array(':openid' => $_W['fans']['from_user'], ':rerid' => $id));
			if (empty($row)) {
				message('我的报名不存在或是已经被删除！');
			}
			$signup = pdo_fetch("SELECT * FROM ".tablename('signup')." WHERE reid = :reid", array(':reid' => $row['reid']));
			$signup['fields'] = pdo_fetchall("SELECT a.title, a.type, b.data FROM ".tablename('signup_fields')." AS a LEFT JOIN ".tablename('signup_data')." AS b ON a.refid = b.refid WHERE a.reid = :reid AND b.rerid = :rerid", array(':reid' => $row['reid'], ':rerid' => $id));
		}
