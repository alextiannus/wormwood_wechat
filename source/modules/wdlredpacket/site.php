...
defined('IN_IA') or exit('Access Denied');
 ...
 class wdlredpacketModuleSite extends WeModuleSite{
 
 
	public $modulename = 'wdlredpacket';
	...
	public function getHomeTitles(){
		global $_W;
		$urls = array();
		$list = pdo_fetchall("SELECT name, id FROM ".tablename('rule')." WHERE weid = '{$_W['weid']}' AND module = 'wdlredpacket'");
		if (!empty($list)) {
			foreach ($list as $row) {
	...
	public function doWebFajiang(){
		...
		pdo_update('wdlredpacket_user',$data,array('id'=>$id));
	...
	public function doWebAwardSet(){
		...
		$row = pdo_fetchall("SELECT award FROM " . tablename('wdlredpacket_award_set') . " WHERE weid={$weid} AND awardid in (0,1,2,3,4,5,6,7,8)");
		...
		if($_W['ispost']){
			...
			foreach($names as $key=>$name){
				...
				$is=pdo_fetch("SELECT * FROM ". tablename('wdlredpacket_award_set'). "WHERE awardid='{$key}' AND weid = {$weid}");
				if(empty($is)){
					pdo_insert('wdlredpacket_award_set',$insert);
				}else{
					pdo_update('wdlredpacket_award_set',$insert,array('id'=>$is['id']));
				}
	...
	public function doWebAwardlist() {
		...
		if (checksubmit('delete')) {
			pdo_delete('wdlredpacket_user', " id  IN  ('".implode("','", $_GPC['select'])."')");
			message('删除成功！', $this->createWebUrl('awardlist', array('id' => $id, 'page' => $_GPC['page'])));
		}
		if (!empty($_GPC['wid'])) {
			$wid = intval($_GPC['wid']);
			pdo_update('wdlredpacket_user', array('status' => intval($_GPC['status'])), array('id' => $wid));
		...
		$sql = "SELECT a.id,  a.award,a.createtime,a.num,a.status, b.nickname, b.mobile FROM ".tablename('wdlredpacket_user')." AS a
		...
		if (!empty($list)) {
			$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('wdlredpacket_user')." AS a
