<?php
		...
		$row = pdo_fetch("SELECT * FROM " .tablename('wdlredpacket_award_set') . " WHERE awardid={$huodeid} AND weid={$_W['weid']}");
		...
		pdo_insert('wdlredpacket_winner', $data);
			...
			$is=pdo_fetch("SELECT * FROM ". tablename('wdlredpacket_user'). "WHERE from_user = '{$fromuser}' AND huodeid='{$huodeid}' AND rid = {$id}");
			
			//print_r($is);
			
			if(empty($is)){
				pdo_insert('wdlredpacket_user',$insert);
			}else{
				$numadd = 1;
				pdo_query("UPDATE  ".tablename('wdlredpacket_user')." SET `num` = `num`+1 WHERE from_user = :from_user AND huodeid={$huodeid} AND rid={$id}",array(':from_user'=>$fromuser));
			}
			
		
		if ($totals>=$wdlredpacket['maxlottery']){
				pdo_query("UPDATE  ".tablename('wdlredpacket_user')." SET count=count-1 , points=points+".$level['key']." WHERE from_user = '{$fromuser}' AND rid=".$id);
				$user['usercont']=$user['usercont']-1;
			
		}else{
			pdo_query("UPDATE  ".tablename('wdlredpacket_user')." SET points=points+".$level['key']." WHERE from_user = '{$fromuser}' AND rid=".$id);
		}
